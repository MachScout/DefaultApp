    ...
    var body: some View {
        GroupBox("Custom association") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                } else if record.creation == .dynamic {
                    Text("macOS generated this type from its filename extension. Its default handler is saved as a Launch Services preference; no type declaration was installed.")
                        .font(.callout).foregroundStyle(.secondary)
                } else {
                    switch state {
                    case .registered:
...
private struct ContentTypeMetadataView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            if record.isDynamic == true {
                MetadataRow(label: "Origin", value: "Dynamic type generated by macOS")
            }
